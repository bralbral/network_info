...
def export_to_mmdb(connection_string, output_file, table="block"):
    ...
    print("Starting export (streaming blocks)...")

    try:
        from mmdb_writer import MMDBWriter
    except ImportError:
        print(
            "Error: mmdb_writer library not found. Please install it with: pip install mmdb-writer"
        )
        sys.exit(1)
    ...
    writer = MMDBWriter(
        ...
        languages=["en"],
        description={"en": "Network block information from RIR databases"},
    )
    ...
    for block in blocks:
        try:
            ...
            data = {
                ...
                "last_modified": block.last_modified.strftime("%Y-%m-%d")
...
def main():
    parser = argparse.ArgumentParser(
        description="Export PostgreSQL table to MMDB format"
    )
    parser.add_argument(
        "-c",
        "--connection",
    ...
    parser.add_argument(
        "-o", "--output", default="block_dump.mmdb", help="Output MMDB file path"
    )
    parser.add_argument("-t", "--table", default="block", help="Table name to export")

    args = parser.parse_args()

    # Add date to output filename if not already present
    if not args.output.endswith(".mmdb"):
        base_name = args.output.rsplit(".", 1)[0] if "." in args.output else args.output
        args.output = f"{base_name}_{datetime.now().strftime('%Y-%m-%d')}.mmdb"
